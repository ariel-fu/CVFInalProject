...
public class Canvas {
	...
	public Canvas(int width, int height) {
		if (width <= 0 || height <= 0) {
			throw new IllegalArgumentException(
					"Out of bounds for width or height: " + width + " width value " + height + " height value.");
		}
	 ...
	 * @throws IllegalArgumentException - if the row and the col are out of bounds / off the canvas
	 */
	...
	public boolean redo() {
		...
		} catch (EmptyStackException e) { // this peek method should throw a EmptyStackException if the stack is
	...
	public void printHistory() {
		...
		for(DrawingChange changes : undoStack) {
			System.out.println("#" + i + ". draw '" + changes.newChar + "' on (" + changes.row + ", " + changes.col + ")");
			i--;
