 ...
 *
 * Each instance stores the results of computing the total number of cases and
 * deaths for the given state or territory.
 * 
...
public class StateSummary {
  ...
  public StateSummary(String stateName, List<DailyStateDataEntry> recordsList) {
    ...
    // calculate the sum and the deaths
    for (int i = 0; i < recordsList.size(); i++) {
      this.totalCases += recordsList.get(i).getCases();
      this.totalDeaths += recordsList.get(i).getDeaths();
    }
    // set the statename
  ...
  /**
   * DO NOT EDIT Returns a comma-separated string representation of this
   * summary. For example: Wisconsin,123,12 Illinois,2390,431
   */
   ...
   * 
   * Alaska 8028 159 Virgin Islands 1548 45 Northern Mariana Islands 321 48
   * 
   */
  public String toFormattedString() {
    return String.format("%30s %8d %8d", getStateName(), getTotalCases(),
        getTotalDeaths());
  }
  ...
  public static void main(String[] args) {
    DailyStateDataEntry w1 = DailyStateDataEntry
        .parse("d1001,Wisconsin,123,12");
    DailyStateDataEntry w2 = DailyStateDataEntry
        .parse("d2245,Wisconsin,239,43");
