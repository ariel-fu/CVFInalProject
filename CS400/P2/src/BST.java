...
public class BST<K extends Comparable<K>, V> implements STADT<K, V> {
  ...
  private int maxHeight(Node curr) {
    if (curr == null) {
      ...
      int left = maxHeight(curr.getLeftNode()); // recur to find the maxHeight
                                                // in the left subtree
      int right = maxHeight(curr.getRightNode()); // recur to find the maxHeight
  ...
  private Node removeHelper(Node curr, K key) {
    if (curr.getKey().compareTo(key) > 0) {
      ...
      if (curr.getLeftNode() == null && curr.getRightNode() == null) {
        ...
        // set the curr node to its only child (right child) and set its only
        // child to
        // null
        curr = curr.getRightNode();
        curr.setRight(null);
      } else if (curr.getRightNode() == null) {
        // set the curr node to its only child (left child) and set its only
        // child to
        // null
  ...
  public void print() {
    List<Node> nodes = this.levelOrderTraversalHelper(root);
    int maxLevel = this.getHeight();

    printHelper(nodes, 0, maxLevel);
  }

  private void printHelper(List<Node> nodes, int currLevel, int maxHeight) {
 

  }

  private void printWhiteSpaces(int numSpaces) {
