...
public class TestBST {
   ...
   * root, left, and right keys to see if insert worked in the other direction.
   */
  @Test
  ...
  void testBST_preOrder() {
    ...
    for (int i = 0; i < expected.size(); i++) {
      if (!resultsPreOrder.get(i).equals(expected.get(i))) {
        ...
        fail("Results @i: " + resultsPreOrder.get(i) + " expected: "
                  + expected.get(i));
      }
    }
  }
  ...
  void testBST_postOrder() {
    try {
      ...
      Integer[] expected = new Integer[] { 0, 15, 10, 30, 20, 70, 85, 90, 80,
          50 };
      for (int i = 0; i < expected.length; i++) {
  ...
  void testBST_level_order() {
    try {
      ...
      Integer[] expected = new Integer[] { 50, 20, 80, 10, 30, 70, 90, 0, 15,
          85 };
      for (int i = 0; i < expected.length; i++) {
  ...
  void testPrint() {
    try {
      ...
      fail("No exceptions should be thrown from 2 inserts and 1 print method");
    } catch (DuplicateKeyException e) {
      fail("No exceptions should be thrown from 2 inserts and 1 print method");
    }
  }
  ...
  /**
   * Tests a insert then a remove then checks if contains can still get it
   */
  @Test
  ...
  /**
   * Tests insert and the remove everything
   */
  @Test
  void test_remove() {
    try {
  ...
  void test_get_valid() {
    try {
      ...
      bst.insert(40, "40");
      bst.insert(230, "230");
      assertTrue(bst.get(40).equals("40"));
    } catch (Exception e) {
      fail("No exceptions should be thrown");
    }
  }
  ...
  /**
   * Tests removing a value and trying to get it
   */
  @Test
  void test_get_removed_value() {
    try {
      bst.insert(20, "20");
      bst.insert(30, "30");
      bst.insert(0, "0");
      ...
      bst.insert(230, "230");
      bst.remove(40);
      bst.get(40); // should throw a KeyNotFoundException
    } catch (KeyNotFoundException e) {

    } catch (Exception e) {
      fail("No exceptions should be thrown other than KeyNotFound"
                + e.getMessage());
    }
  }
