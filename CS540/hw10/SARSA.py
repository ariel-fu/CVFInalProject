    ...
    for i in range(EPISODES):
        ...
        while done == False:
            ...
            # else:
                ...
                # take the action with the highest Q_table(s, a) value
                prediction = np.array([Q_table[(next_state, index)] for index in range(env.action_space.n)])
                next_action = np.argmax(prediction)
            
            # update Q values           
            # Q = Q_table(state, action) + alpha(reward + gamma((Q_table(next state, next action))) - Q_table(state, action))
            learned_value = DISCOUNT_FACTOR * Q_table[next_state, next_action]
