...
public class Main extends Application {
  ...
  private ComboBox<String> farmCombo = new ComboBox<String>(FXCollections.observableArrayList());
  ...
  private ComboBox<String> monthCombo = new ComboBox<String>(FXCollections.observableArrayList());
  ...
  public void start(Stage arg0) throws Exception {
    ...
    Label lable2 = new Label(
        "Welcome! Start by entering the file name below, click Submit," + "then click Continue");
  ...
  private void choiceScene(Stage arg0, Button continueButton) throws IOException {
    ...
    farmReportSubmitButton.setOnAction(e -> {
      for (int i = 0; i < hashMap.size(); i++) {
        if (farmCombo.getValue() == allFarms.get(i).getID()) {
          arg0.setScene(this.getFarmReportSceneAfterUserChosesAFarm(arg0, allFarms.get(i)));
        }
      }
    });

    // If the user decides to look at the monthly report
    monthButton.setOnAction(e -> arg0.setScene(getMonthlyReportScene(arg0, choiceScene)));
    ...
    monthReportSubmitButton.setOnAction(e -> {
      if (getMonthNum(monthCombo.getValue()) == -1) {
        ...
        arg0.setScene(this.getMonthlyReportSceneAfterUserChosesAMonth(arg0, "January"));
      } else if (getMonthNum(monthCombo.getValue()) == 2) {
        arg0.setScene(this.getMonthlyReportSceneAfterUserChosesAMonth(arg0, "February"));

      } else if (getMonthNum(monthCombo.getValue()) == 3) {
        arg0.setScene(this.getMonthlyReportSceneAfterUserChosesAMonth(arg0, "March"));
      } else if (getMonthNum(monthCombo.getValue()) == 4) {
        arg0.setScene(this.getMonthlyReportSceneAfterUserChosesAMonth(arg0, "April"));
      } else if (getMonthNum(monthCombo.getValue()) == 5) {
        arg0.setScene(this.getMonthlyReportSceneAfterUserChosesAMonth(arg0, "May"));
      } else if (getMonthNum(monthCombo.getValue()) == 6) {
        arg0.setScene(this.getMonthlyReportSceneAfterUserChosesAMonth(arg0, "June"));
      } else if (getMonthNum(monthCombo.getValue()) == 7) {
        arg0.setScene(this.getMonthlyReportSceneAfterUserChosesAMonth(arg0, "July"));
      } else if (getMonthNum(monthCombo.getValue()) == 8) {
        arg0.setScene(this.getMonthlyReportSceneAfterUserChosesAMonth(arg0, "August"));
      } else if (getMonthNum(monthCombo.getValue()) == 9) {
        arg0.setScene(this.getMonthlyReportSceneAfterUserChosesAMonth(arg0, "September"));
      } else if (getMonthNum(monthCombo.getValue()) == 10) {
        arg0.setScene(this.getMonthlyReportSceneAfterUserChosesAMonth(arg0, "October"));
      } else if (getMonthNum(monthCombo.getValue()) == 11) {
        arg0.setScene(this.getMonthlyReportSceneAfterUserChosesAMonth(arg0, "November"));
      } else {
        arg0.setScene(this.getMonthlyReportSceneAfterUserChosesAMonth(arg0, "December"));
      }
    });

    // If the user decides to look at the annual report
    annualButton.setOnAction(e -> arg0.setScene(getAnnualReportScene(arg0, choiceScene)));
   ...
   */
  private Scene getFarmReportSceneAfterUserChosesAFarm(Stage arg0, Farm chosenFarm) {
    // Pie Chart
  ...
  private Scene getAnnualReportScene(Stage arg0, Scene choice) {
    ...
    thirdBox.getChildren().addAll(thirdText, hBoxAll, charts, goBackAndSave, spaceAndNotes);
    ...
    saveReport.setOnAction(e -> {
      ...
      try {
        csv.writeToAFile("Annual Report", annualReportSave.min, annualReportSave.max,
            annualReportSave.average, annualReportSave.percents, annualReportSave.percentLabels);
        System.out.println(
            annualReportSave.min + " " + annualReportSave.max + " " + annualReportSave.average + " "
  ...
  private VBox getTableForAnnualReportScene() {
    ...
    farmTable.setCellValueFactory(new PropertyValueFactory<TableInner, String>("name"));
    percent.setCellValueFactory(new PropertyValueFactory<TableInner, String>("percent"));
    totle.setCellValueFactory(new PropertyValueFactory<TableInner, String>("weight"));
   ...
   * @return
   */
  private Scene getMonthlyReportSceneAfterUserChosesAMonth(Stage arg0, String chosenMonth) {
    // ComboBox
    Label monthLabel = new Label("Month     ");
    HBox hBox = new HBox();
    Label notes = new Label("NOTE: If you want to look at another month, use the drop down again. "
        + "Click the Go Back button to go back and "
        + "look at more reports. If you would like to save this info to look "
        + "back later in reportData file, click the Save to the file " + "button.");
    ...
    tableSpace.getChildren().addAll(getTableForMonthlyReportScene(chosenMonth), spacing);
    ...
    // Makes the forthScene
    VBox forthBox = new VBox();
    forthBox.getChildren().addAll(forthText, hBox, monthReportSubmitButton, notes, charts, hbox);
    ...
    saveReport.setOnAction(e -> {
      ...
      try {
        csv.writeToAFile("Monthly Report", monthlyReportSave.min, monthlyReportSave.max,
            monthlyReportSave.average, monthlyReportSave.percents, monthlyReportSave.percentLabels);
      } catch (IOException e1) {
        // TODO Auto-generated catch block
        e1.printStackTrace();
      }
  ...
  /**
   * Helper method for getTableForMonthlyReportScene() to return a list of info to
   * put in the table
   * 
   * @param chosenMonth - the month chosen by the user
   * @return a list of info to pul in the table
   */
  private ObservableList<TableInner> getListForTableForMonthlyReportScene(String chosenMonth) {
    ObservableList<TableInner> tableInner = FXCollections.observableArrayList();
    MonthlyReport monthly = new MonthlyReport(hashMap, new Year(2019), getMonthNum(chosenMonth));
    monthly.runReport();
    for (int i = 0; i < allFarms.size(); i++) {
   ...
   * @return the data range report scene with table
   */
  private Scene getDataRangeReportSceneAfterPickingDate(Stage arg0, LocalDate date1,
      LocalDate date2) {
    Text fifthText = new Text();
    fifthText.setText("Data Range Report");
    fifthText.setFill(Color.BLACK);
    ...
    Label note = new Label(
        "NOTE: If you want to look at report of different dates, please go back and come "
            ...
            + "save this info to look " + "back later in reportData file, click the Save "
            + "to the file button.");
    // Go back to choices
    VBox fifthBox = new VBox();
    fifthBox.getChildren().addAll(fifthText, firstToSecondDate,
        getTableForDataRangeReportScene(date1, date2), goBackAndSave, spaceBetweenTableAndText,
        note);
    ...
    saveReport.setOnAction(e -> {
      ...
      try {
        csv.writeToAFile("Data Range Report", dateRangeReportSave.min, dateRangeReportSave.max,
            dateRangeReportSave.average, dateRangeReportSave.percents,
            dateRangeReportSave.percentLabels);
      } catch (IOException e1) {
   ...
   */
  private VBox getTableForDataRangeReportScene(LocalDate date1, LocalDate date2) {
    // Table for farm, percentage, and weight
    final TableView<TableInner> table = new TableView<>();
  ...
  /**
   * Helper method for getTableForDataRangeReportScene() to return a list of info
   * to put in the table
   * 
  ...
  private ObservableList<TableInner> getListForTableDataRangeReportScene(LocalDate date1,
    ...
    for (int i = 0; i < allFarms.size(); i++) {
      ...
      tableInner
          .add(new TableInner(dateRangeReport.percentLabels.get(i), percentDouble, weightInt));
    }
    return tableInner;
  }
  ...
  /**
   * All scene has a button to go back to the choice scene for the user to pick to
   * look at a different data. This method returns that button for all scene
   * methods.
   * 
   * @param arg0 - arg
