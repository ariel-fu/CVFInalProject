...
public class FourBytes {
  ...
  /**
   * Changes a sequence of consecutive bits within this FourBytes object to
   * reflect the specified value. Consider for example the a call of
   * setBits(3,4,13) on the following FourBytes object initialized to 0:
   * 
   * bitstring : 00000000 00000000 00000000 00000000 bitstring.setBits(3,8,13):
   * 00000000 00000000 00000101 00000000 changed bits: ^^^ <- offset of 8 bits 13
   * in binary is 1101, but only 3 right-most/least-significant bits are set If
   * you'd like to test this, the resulting bit string: 101 00000000 = 1280
   * 
   ...
   *                     an offset of 0, changes only the numberOfBits right-most
   *                     bits
   * @param value        to store in the specified portion of this FourBytes
   *                     object note that only the numberOfBits least significant
   *                     bits are used
   */
  public void setBits(int numberOfBits, int offset, int value) {
    ...
    if(numberOfBits >= 32)
      mask = -1; // fill entire mask with 1s
  ...
  /**
   * Retrieves the int value represented by a sequence of consecutive bits within
   * this FourBytes object. Consider for example the a call of getBits(4,10) on
   * the following FourBytesObject:
   * 
   * bitstring: 00000000 00000000 00110100 00000000 read bits: ^^^^ <- offset of
   * 10 bits returns 13 corresponding to binary sequence 1101 If you'd like to
   * test this, the initial bit string: 11010000000000 = 13312
   * 
   * @param numberOfBits is the total number of bits to access
   * @param offset       distance left from th right-most/least significant bit an
   *                     offset of 0, reads only the numberOfBits right-most bits
   * @return int representation of the specified bits within this FourBytes object
   *         note that any extra higher significant bits in this int are
   *         initialized to zero
   */
  ...
  /**
   * Retrieve the char value represented by this FourByte object. (note that chars
   * are only 16 bit, and the right-most/least-significant bits are used for this.
   * 
  ...
  /**
   * Retrieve a string representation showing the both int and binary
   * representations of this FourByte object.
   */
