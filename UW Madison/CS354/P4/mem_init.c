...
void Initialize_Memory_Allocator(int size)
{
    ...
    while (size % 16)
        size++; // round up to a size divisible by 16
    ...
    while (start % 16)
        start++;                          // first payload must be an address divisible by 16
...
// test
int getblocksize(BLOCK_HEADER *header)
{
    return header->size_alloc & 0xFFFFFFFE;
}
BLOCK_HEADER *getnextheader(BLOCK_HEADER *header)
{
    int block_size = getblocksize(header);
    header = (BLOCK_HEADER *)((unsigned long)header + 8);
    return header;
}
void *getuserpointer(BLOCK_HEADER *header)
{
    return (BLOCK_HEADER *)((unsigned long)header + 8);
}
int islast(BLOCK_HEADER *header)
{
...
void print_block(void *ptr)
{
    ...
    header = curr;
    user_pointer = (void *)((unsigned long)curr + 8);
    block_size = curr->size_alloc;
    payload = curr->payload;
    padding = (block_size & 0xFFFFFFFE) - 8 - payload;

    printf("header: %p, user pointer: %p, block size: %d, payload: %d, padding: %d\n", header, user_pointer, block_size, payload, padding);
}