...
class TestTeeko2Player:
    ...
    def game_value(self, state, ai_piece):
        ...
        for i in range(1,4):
            for j in range(1,4):
                if state[i][j] == ' ' and state[i-1][j-1] == state[i-1][j+1] == state[i+1][j-1] == state[i-1][j+1]:
                    return 1 if state[i-1][j+1]==ai_piece else -1
    ...
    def test_code(self):
        ...
        if self.later_time_exceeded:
            print('-5: make_move() takes more than 5 secs during continued gameplay after drop phase')
            self.score -= 5

        if self.ai_wins < 2/3 * self.n_expts:
            print('-25: AI loses', self.n_expts - self.ai_wins, 'times out of', self.n_expts)
            self.score -= 25
...
def main():
    ...
    score = test_teeko2_player.test_code()
